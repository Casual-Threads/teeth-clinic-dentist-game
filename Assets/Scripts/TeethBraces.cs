...
{
    none, Clipper, Brush, Excavator, MiniMicro, SecondDrill, FillSpoon, TeethLaser, PourWater, WateringOut, Germs
}
...
public class TeethBraces : MonoBehaviour
{
    ...
    public GameObject brush, excavator, miniMicro, secondDrill, spoonWithPot, teethLaser, waterPipe, waterOutPump;
    [Header("Items Layers")]
    ...
    public Image[] crackTeethLayer, blackDamagedTeethLayer, yellowDamagedTeethLayer;
    [Header("Sprites")]
    ...
    {   
        ...
        else if(action == TeethBracesActionPerform.Excavator)
        {
            ...
            StartCoroutine(EnableOrDisable(0.5f, miniMicro, true));
            AfterTaskDonePerform();
            for (int i = 0; i < crackTeethLayer.Length; i++)
            {
                crackTeethLayer[i].transform.GetChild(0).gameObject.SetActive(true);
                if (downParent)
                {
                    crackTeethLayer[i].transform.parent = downParent;
                }
            }
        ...
        {
            taskDoneParticle.gameObject.SetActive(true);
            StartCoroutine(EnableOrDisable(0.5f, miniMicro, false));
        ...
        else if(action == TeethBracesActionPerform.SecondDrill)
        {
            taskDoneParticle.gameObject.SetActive(true);
        ...
        else if(action == TeethBracesActionPerform.FillSpoon)
        {
            taskDoneParticle.gameObject.SetActive(true);
        ...
        {
            taskDoneParticle.gameObject.SetActive(true);
            action = TeethBracesActionPerform.PourWater;
        ...
        else if (action == TeethBracesActionPerform.PourWater)
        {
            taskDoneParticle.gameObject.SetActive(true);
            action = TeethBracesActionPerform.WateringOut;
    ...
    IEnumerator AfterTaskColliderAndIndicationsOn()
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            if (action == TeethBracesActionPerform.Excavator)
            {
    ...
    IEnumerator ChangeActionType()
    {
        ...
        {
            action = TeethBracesActionPerform.SecondDrill;
        }
        else if (action == TeethBracesActionPerform.SecondDrill)
        {
            action = TeethBracesActionPerform.FillSpoon;
        }
